...
var SmartNotifs = (() => {
  ...
  function getCurrentInsight() {
    ...
    try {
      ...
      // ─── تنبيه 7: الزكاة (لو مكتمل الحول) ───
      if (window.ZakatCalculator) {
        const hawl = window.ZakatCalculator.checkHawl();
        if (hawl?.isComplete) {
          insights.push({
            type: 'zakat-due',
            priority: 9,
            icon: '🤲',
            title: 'حان وقت الزكاة',
            message: 'مرّ حول كامل على نصابك — بادر بإخراج الزكاة',
            action: { label: 'احسب الزكاة', tab: 'zakat' }
          });
        }
      }
      
      // ─── تنبيه 8: تشجيع للسلسلة ───
