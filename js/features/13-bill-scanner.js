    ...
    dialog.innerHTML = `
      <div class="scan-review">
        ...
        <label class="scan-field">
          <span>المبلغ</span>
          <input type="number" id="scanAmt" value="${result.bestAmount || ''}" step="0.01">
        </label>
        
        <label class="scan-field">
          <span>الاسم/المتجر</span>
          <input type="text" id="scanName" value="${(result.merchant || '').replace(/"/g, '')}" maxlength="60">
        </label>
        
        <label class="scan-field">
          <span>التصنيف</span>
          <input type="text" id="scanCat" value="${result.suggestedCategory || '➕'}" maxlength="4">
        </label>
        ...
        ${result.amounts.length > 1 ? `
          <div class="scan-suggestions">
            ...
            ${result.amounts.slice(1).map(a => 
              `<button class="scan-amt-pick" data-amt="${a.value}">${a.value}</button>`
            ).join('')}
          </div>
        ` : ''}
