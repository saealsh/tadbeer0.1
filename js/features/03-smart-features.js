  ...
  const Gift = (() => {
    const REWARDS = [
      ...
      { type: 'quote', icon: '📜', weight: 10, label: 'حكمة اليوم', quotes: [
        ...
        '"المال خادم جيد ولكنه سيد سيء" - فرانسيس بيكون',
        '"لا تدخر ما تبقى بعد الإنفاق، بل أنفق ما تبقى بعد الادخار" - وارن بافيت',
        '"الثروة الحقيقية هي ما تستطيع شراءه بدون مال" - مثل عالمي',
        '"الفقير ليس من قلّ ماله، بل من كثرت حاجاته" - حكمة'
      ] },
      // Rare (25%)
  ...
  const Simulator = (() => {
    const SCENARIOS = [
      ...
      },
      {
        id: 'travel',
        emoji: '✈️',
        text: 'لو وفّرت لرحلة سفر',
        defaultAmount: 500,
        defaultYears: 1,
        label1: 'ادخار شهري',
        label2: 'المدة (سنوات)',
        calc: (a, y) => a * 12 * y,
        headline: 'ستجمع للسفر',
        period: (y) => `في ${y} ${y === 1 ? 'سنة' : 'سنوات'}`,
        insight: (total) => `رحلة محلية: ٣٠٠٠-٧٠٠٠ ريال، رحلة خارجية: ١٠٠٠٠-٣٠٠٠٠ ريال. ابدأ من اليوم!`
      }
    ];
    ...
    function getEquivalents(total) {
      const equivs = [];
      // رحلة محلية (تقريب 4000 ريال)
      const localTrips = Math.floor(total / 4000);
      if (localTrips >= 1 && localTrips <= 100) equivs.push({ icon: '🏖️', text: `${localTrips} ${localTrips === 1 ? 'رحلة محلية' : 'رحلات محلية'}` });

      // سيارة مستعملة (30000 ريال)
      if (total >= 30000 && total < 60000) {
        const usedCars = Math.floor(total / 30000);
        equivs.push({ icon: '🚙', text: `${usedCars} ${usedCars === 1 ? 'سيارة مستعملة' : 'سيارات مستعملة'}` });
      }
      ...
      // رحلات خارجية (8000)
      const intlTrips = Math.floor(total / 8000);
      if (intlTrips >= 1 && intlTrips <= 30) equivs.push({ icon: '✈️', text: `${intlTrips} ${intlTrips === 1 ? 'رحلة خارجية' : 'رحلات خارجية'}` });

      // أجهزة جديدة (3000 لاب توب/جوال متوسط)
      const devices = Math.floor(total / 3000);
      if (devices >= 1 && devices <= 50) equivs.push({ icon: '💻', text: `${devices} ${devices === 1 ? 'جهاز جديد' : 'أجهزة جديدة'}` });

      // سيارة جديدة (60000)
      if (total >= 60000) {
        const newCars = Math.floor(total / 60000);
        equivs.push({ icon: '🚗', text: `${newCars} ${newCars === 1 ? 'سيارة جديدة' : 'سيارات جديدة'}` });
      }
