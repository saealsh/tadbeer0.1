...
var Renderers = (() => {
    ...
    if (!d.income.length) {
      list.innerHTML = `
        <div class="section-empty">
          ...
          <div class="section-empty-text">سجّل أول دخل لتبدأ التحليل</div>
          <button class="section-empty-btn" data-toggle-form="incomeForm">+ أضف مصدر دخل</button>
        </div>`;
    } else {
      list.innerHTML = '';
  ...
  function fixed() {
    ...
    if (!d.fixed.length) {
      list.innerHTML = `
        <div class="section-empty">
          <div class="section-empty-icon">🏠</div>
          <div class="section-empty-text">إيجار، اشتراكات، فواتير...</div>
          <button class="section-empty-btn" data-toggle-form="fixedForm">+ أضف مصروف ثابت</button>
        </div>`;
    } else {
      list.innerHTML = '';
  ...
  function yearly() {
    // ═══ NEW: تفويض كامل لـ YearlyCharts module ═══
    // تم استبدال الـ rendering القديم في v4.4
    if (window.YearlyCharts?.render) {
      window.YearlyCharts.render();
    }
  }
